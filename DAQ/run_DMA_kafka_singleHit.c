/**
 * READ DATA FROM DMA AND PRODUCE EVERY HIT OF DATA-TRANSFER TO KAFKA (8 Bytes/message)
 *
 * COMPILE INCLUDING THE LIBRARIES lpthread lrdkafka lz lrt
 * gcc run_test_kafka_singleHit.c -lrdkafka -lz -lpthread -lrt -o run_test_kafka_singleHit
 */
  ...
  while (run && count--) {
    ...
    if (rc>0) {
      ...
      while(ith_word++ < buffer_words-1) {
        ...
        uint64_t THE_HIT[1];
        THE_HIT[0] = ((uint64_t *)buffer)[ith_word];
        size_t THE_HIT_len = sizeof(THE_HIT[0]); 
        ...
        retry:
          ...
          if (rd_kafka_produce(rkt,                   // Topic object
                               ...
                               THE_HIT, 
                               THE_HIT_len,           // Message payload (value) and length 
                               NULL,
