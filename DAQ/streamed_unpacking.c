...
static int test_dma(char *devicename, uint32_t addr, uint32_t size, uint32_t offset, uint32_t count, char *filenameraw, char *filenameunpack);
...
int main(int argc, char* argv[])
{
  ...
  test_dma(device, address, size, offset, count, filenameraw, filenameunpack);
}
...
static int test_dma(char *devicename, uint32_t addr, uint32_t size, uint32_t offset, uint32_t count, char *filenameraw, char *filenameunpack)
{
  ...
  if (filenameraw) {
    fileraw_fd = open(filenameraw, O_RDWR | O_CREAT | O_TRUNC | O_SYNC, 0666);
    assert(fileraw_fd >= 0);
  }

  if (filenameunpack) {
    fileunpack_fd = open(filenameunpack, O_RDWR | O_CREAT | O_TRUNC | O_SYNC, 0666);
    assert(fileunpack_fd >= 0);
    char header[] = "HEAD,FPGA,TDC_CHANNEL,ORBIT_CNT,BX_COUNTER,TDC_MEAS\n" ;
    write(fileunpack_fd, header, sizeof(header)-1);
  }
  ...
  while (count--) {
    ...
    if ((rc > 0) && (rc < size)) {
      printf("Short read of %d bytes into a %d bytes buffer\n", rc, size);
    }

    /* write data to file */
    if ((fileraw_fd >= 0) & (no_write == 0)) {      

      /* write raw buffer to file */
      rc = write(fileraw_fd, buffer, size);
      assert(rc == size);

      /* write unpacked buffer to file */
      int buffer_words = size/sizeof(dummy);
      int ith_word = -1;

      /* unpack data on-the-fly */
      while(ith_word++ < buffer_words-1){

        //if ( ((uint64_t *)buffer)[ith_word] == 0) continue;
        uint32_t TDC_MEAS                = -1 + (uint32_t)(((uint64_t *)buffer)[ith_word] >> 0  ) & 0x1F;	// from 1 to 30 -> 0 to 29
        ...
        char wordbuffer[100];
        int len = sprintf(wordbuffer,"%"PRIu32",%"PRIu32",%"PRIu32",%"PRIu32",%"PRIu32",%"PRIu32"\n",HEAD,FPGA,TDC_CHANNEL,ORBIT_CNT,BX_COUNTER,TDC_MEAS);
//        write(fileunpack_fd, wordbuffer, len);
