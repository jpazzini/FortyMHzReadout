/**
 * READ DATA FROM DMA --- FOR OBDT TESTING PURPOSES 
 *
  ...
  while (run && count--) {
    ...
    if (rc>0) {
      ...
      while(ith_word++ < buffer_words-1) {
        ...
        uint32_t TDC_CHANNEL             =      (uint32_t)(((uint64_t *)buffer)[ith_word] >> 49 ) & 0x1FF;   
        uint32_t FPGA                    =      (uint32_t)(((uint64_t *)buffer)[ith_word] >> 58 ) & 0xF  ;
        uint32_t HEAD                    =      (uint32_t)(((uint64_t *)buffer)[ith_word] >> 62 ) & 0x3  ;

        //INFO ---> NO SHIFTING OF CHANNELS 
        //INFO ---> NO SHIFTING OF TDC
