...
def meantimer(message):
  ...
  if USE_INTTRIG:
    ...
    print 'looping over events', len(events)
    jobs = []
    ...
    for event, df in events:
      for theSL in range(4):
        out[event+theSL/10.] = []
        if not df.loc[df['SL']==theSL].shape[0] < 5: # at least to have one quadruplet plus the possible hit 139   
    ...
    for event, df in events:
      for theSL in range(4):
        if len(out[event+theSL/10.]) > 0:
          allhits.loc[allhits['ORBIT_CNT']==event,'TIME0'] = np.mean(out[event+theSL/10.]) #min(out[event+theSL/10.])
    print 'done looping'
